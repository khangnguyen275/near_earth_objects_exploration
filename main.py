...
def make_parser():
    ...
    inspect = subparsers.add_parser(
        'inspect', description="Inspect an NEO by primary designation or by \
            name.")
    ...
    repl.add_argument(
        ...
        '--aggressive',
        action='store_true',
        help="If specified, kill the session whenever a project file is \
            modified.")
...
class NEOShell(cmd.Cmd):
    ...
    def precmd(self, line):
        ...
        if changed:
            print(
                "The following file(s) have been modified since this \
                    interactive session began: "
                f"{', '.join(str(f.relative_to(PROJECT_ROOT)) for f in changed)}.",
                file=sys.stderr)
            if not self.aggressive:
                print(
                    "To include these changes, please exit and restart this \
                        interactive session.",
                    file=sys.stderr)
            else:
                print(
